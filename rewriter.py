...
4. 원문에 [이미지]가 있는 위치와 개수를 최대한 동일하게 유지하세요. 각 이미지 위치에 [이미지: 검색키워드] 형태로 표시하세요. 검색키워드는 해당 문맥에서 실제로 찾을 수 있는 구체적인 영문 검색어여야 합니다. 인물명, 브랜드명, 제품명 등 고유명사를 반드시 포함하세요. 예: [이미지: paris hilton velvet tracksuit 2000s], [이미지: jennie blackpink lace bodysuit bangkok concert], [이미지: zara poet core loafer 2026 spring]
