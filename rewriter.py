...
4. 원문에 [이미지]가 있는 위치와 개수를 최대한 동일하게 유지하세요. 각 이미지 위치에 [이미지: 검색키워드] 형태로 표시하세요. 검색키워드는 해당 위치에 어울리는 이미지를 찾기 위한 영문 검색어입니다. 예: [이미지: korean idol stage performance], [이미지: spring fashion loafer shoes]
